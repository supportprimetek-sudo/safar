...
export const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await register({ fullName, email, phone, password, emergencyContact });
      navigate('/');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-safar-bg flex flex-col justify-center px-6 py-12">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-2">
        ...
        <h2 className="text-3xl font-black tracking-tight text-white">Create Rider Account</h2>
        <p className="text-sm text-safar-textMuted">Join SAFAR for safe & smart urban mobility.</p>
      </div>
      ...
      <div className="mt-6 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-safar-card p-8 rounded-3xl border border-white/10 shadow-2xl space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <span>{loading ? 'Creating Account...' : 'Register as Rider'}</span>
