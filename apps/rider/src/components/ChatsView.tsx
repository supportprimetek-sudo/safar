...
export const ChatsView: React.FC<ChatsViewProps> = ({ currentRide, socket }) => {
  ...
  const quickReplies = [
    "I'm standing at pickup spot",
    "Arriving at pickup spot",
    "Please wait at entry gate",
    "In traffic, 2 mins",
    "Call me on phone",
  ];
  ...
  const driverName = currentRide?.driver?.user?.fullName || currentRide?.driver?.phone || 'Assigned Driver Partner';
  const driverVehicle = currentRide?.driver?.vehicleType?.name || 'SAFAR Partner';
  const driverPhone = currentRide?.driver?.phone || currentRide?.driver?.user?.phone || '+919876543210';
      ...
      <div className="sticky top-0 z-30 pt-[max(2.5rem,env(safe-area-inset-top,32px))] pb-2 bg-[#11151D] border-b border-white/10 -mx-4 px-4 mb-2">
        <div className="bg-safar-card p-3.5 rounded-3xl border border-white/10 shadow-lg flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-extrabold text-white text-base leading-tight">{driverName}</h3>
              <p className="text-xs text-safar-teal font-bold flex items-center mt-0.5">
                ...
                {currentRide ? `${driverVehicle} • Active Chat` : 'Ride Chat Active'}
              </p>
            </div>
          </div>
      ...
      <div className="flex-1 my-4 space-y-3 overflow-y-auto px-1 max-h-[calc(100vh-16rem)]">
        {!currentRide && messages.length === 0 ? (
          <div className="h-48 flex flex-col items-center justify-center text-center p-6 space-y-3 bg-safar-card/50 rounded-3xl border border-white/5 my-auto">
            ...
            <h4 className="font-extrabold text-white text-base">No Active Ride Chat</h4>
            <p className="text-xs text-safar-textMuted max-w-xs">
              Book a ride to start chatting live with your driver partner in real-time.
            </p>
          </div>
        ) : (
          messages.map((msg) => {
