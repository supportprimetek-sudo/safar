...
export const MapComponent: React.FC<MapProps> = ({
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Sleek Floating Guidance Pill (Positioned cleanly below top SAFAR header) */}
      <div className="absolute top-[max(5.5rem,calc(env(safe-area-inset-top,32px)+3.5rem))] left-1/2 -translate-x-1/2 z-20 bg-[#202631]/95 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/15 shadow-xl flex items-center space-x-1.5 text-[11px] font-extrabold text-white/90 pointer-events-none whitespace-nowrap max-w-[90vw] truncate">
        <MapPin className="w-3 h-3 text-[#35D0B0] animate-pulse flex-shrink-0" />
        <span className="truncate">{isPinningMode ? 'Move map to position central drop pin' : 'Tap map or drag marker to set drop location'}</span>
      </div>
      ...
      {isPinningMode && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-full z-30 pointer-events-none flex flex-col items-center">
          <div className="bg-[#E53935] text-white px-2.5 py-0.5 rounded-full text-[10px] font-black shadow-2xl mb-1 border border-white/20 whitespace-nowrap animate-pulse">
            Drop Location Here
          </div>
          <div className="w-8 h-8 rounded-full bg-[#E53935] border-3 border-white flex items-center justify-center shadow-2xl text-white font-black text-xs">
            D
          </div>
      ...
      <button
        ...
        className="absolute top-[max(5.5rem,calc(env(safe-area-inset-top,32px)+3.5rem))] right-3 z-20 w-9 h-9 bg-[#202631]/95 backdrop-blur-md border border-white/15 text-[#35D0B0] hover:text-white rounded-xl flex items-center justify-center shadow-xl active:scale-95 transition-all group"
      >
        {locatingGps ? (
          <Loader2 className="w-4 h-4 animate-spin text-[#35D0B0]" />
        ) : (
