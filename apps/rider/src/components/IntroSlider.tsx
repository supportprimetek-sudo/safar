...
import { ArrowRight, Check, MapPin, Shield, Zap, Sparkles } from 'lucide-react';
...
export const IntroSlider: React.FC<IntroSliderProps> = ({ onFinish }) => {
  ...
  const slides: Slide[] = [
    {
      id: 1,
      title: 'Fastest City Pickups',
      subtitle: 'Ride Moto, Auto, Sedan & SUV',
      description: 'Book rides in under 15 seconds with verified local drivers, live GPS tracking & instant ETA estimates.',
      category: 'Instant Rides',
    ...
    },
    {
      ...
      subtitle: '1-Tap Drop Location Auto-Fill',
      description: 'Explore curated city landmarks, airports, malls, and train stations. Tap any photo to auto-fill drop location!',
    ...
    },
    {
      ...
      subtitle: 'Zero Hidden Fees & 24/7 Helpline',
      description: 'Enjoy guaranteed fare estimates, in-app driver chat, emergency contact safety SOS, and cash or UPI payments.',
  ...
  const handleNext = () => {
    if (currentIndex < slides.length - 1) {
      ...
      localStorage.setItem('safar_intro_seen', 'true');
      onFinish();
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#11151D] flex flex-col justify-between p-5 pt-[max(2.5rem,env(safe-area-inset-top,36px))] pb-[max(2.25rem,env(safe-area-inset-bottom,28px))] max-w-lg mx-auto animate-fade-in">
      ...
      <div className="space-y-4 pb-2 relative z-10">
        ...
        >
          <span>{currentIndex === slides.length - 1 ? 'Get Started Now' : 'Next'}</span>
